...
SELECT order_status, COUNT(order_date) AS num_orders 
FROM orders 
GROUP BY order_status;

-- What are the dates of the orders and how long do they take to ship?
SELECT order_id, order_status, order_date, shipped_date, DATEDIFF(shipped_date, order_date) AS days_to_ship 
FROM orders 
WHERE order_status IN (3, 4) AND shipped_date IS NOT NULL;

-- On average, how long do they take for orders to ship?
SELECT AVG(DATEDIFF(shipped_date, order_date)) AS days_to_ship 
FROM orders 
...
SELECT product_id, COUNT(quantity) AS quantity_per_order 
FROM order_items 
GROUP BY product_id 
ORDER BY quantity_per_order DESC 
LIMIT 10;
...
SELECT store_id, product_id, MIN(quantity) AS min_inventory 
FROM stocks 
GROUP BY store_id, product_id 
ORDER BY min_inventory;